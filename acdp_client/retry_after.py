...
absent or unparseable. Lives in ``acdp_client`` (rather than
``playground``) so the registry token client can honour a cooperative
rate-limit response without depending on the playground package;
``playground.retry_after`` re-exports it for backwards compatibility.
"""
